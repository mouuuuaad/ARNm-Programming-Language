            ...
            /* Field access on struct/actor */
            {
                ...
                if (obj->kind == TYPE_ACTOR) {
                    ...
                    if (!found) {
                        ...
                        /* Mangled name: Actor_method */
                        /* Warning: this constructs a temporary string. */
                        char mangled[256];
                        /* Need actor name. We only have type. Type has name if TYPE_ACTOR. */
                        /* Type struct needs name field (it has it for actor/struct). */
                    ...
                    sema_error(ctx, expr->as.field.common.span, "field access on non-actor");
                     result = type_error(&ctx->type_arena);
                }
            }
            break;
